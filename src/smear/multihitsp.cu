...
public:
  MultiHitSP(gauge &arrayin, gauge &arrayout, RNG &randstates, int mu, int nhit)
  ...
  void stat() {
    COUT << "MultiHitSP:  " << time() << " s\t" << bandwidth() << " GB/s\t"
         << flops() << " GFlops" << endl;
  }
...
template <class Real>
void ApplyMultiHitSpace(gauge array, gauge arrayout, RNG &randstates,
                        int nhit) {
...
template void ApplyMultiHitSpace<double>(gauged array, gauged arrayout,
                                         RNG &randstates, int nhit);
