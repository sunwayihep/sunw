...
/**
        @brief CUDA Kernel to perform overrelaxation in a even/odd lattice
   order.
...
template <class Real> void OverRelaxation<Real>::SetFunctionPtr() {
  ...
  if (kernel_pointer == NULL)
    errorCULQCD(
        "No kernel OverRelaxation function exist for this gauge array...");
}
...
template <class Real> void OverRelaxation<Real>::Run() { Run(0); }
template <class Real> double OverRelaxation<Real>::time() { return timesec; }

template <class Real> void OverRelaxation<Real>::stat() {
  COUT << "OverRelaxation:  " << time() << " s\t" << bandwidth() << " GB/s\t"
       << flops() << " GFlops" << endl;
}

template <class Real> long long OverRelaxation<Real>::flop() const {
// NEEEDDDDD TO RECOUNT THIS PART!!!!!!!!!!!!!!!!!!!!!!!!!
#if (NCOLORS == 3)
  long long stapleflop = 2268LL;
  long long ThreadFlop = (7LL * array.getNumFlop(true) +
                          array.getNumFlop(false) + stapleflop + 843LL);
#else
  long long overflop = NCOLORS * NCOLORS * NCOLORS +
                       (NCOLORS * (NCOLORS - 1) / 2) * (17 + 112 * NCOLORS);
...
template <class Real> long long OverRelaxation<Real>::bytes() const {
#ifdef MULTI_GPU
...
template <class Real> double OverRelaxation<Real>::flops() {
  return ((double)flop() * 8 * 1.0e-9) / timesec;
}
template <class Real> double OverRelaxation<Real>::bandwidth() {
