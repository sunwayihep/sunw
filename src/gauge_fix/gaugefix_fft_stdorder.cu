...
static inline __host__ __device__ void getCoords4(int x[4], const int id,
                                                  const int X[4]) {
  x[3] = id / (X[0] * X[1] * X[2]);
  x[2] = (id / (X[0] * X[1])) % X[2];
  x[1] = (id / X[0]) % X[1];
...
neighborIndexMinusOne(int x[4], const int mu, const int X[4]) {
  int y[4];
...
static inline __host__ __device__ int
neighborIndexPlusOne(int x[4], const int mu, const int X[4]) {
  int y[4];
  for (int dir = 0; dir < 4; dir++)
    y[dir] = x[dir];
  y[mu] = (y[mu] + 1) % X[mu];
...
__global__ void kernel_calc_Fg_theta_delta(complex *array, complex *res_save,
  ...
  if (idx < DEVPARAMS::Volume) {
    ...
    for (int nu = 0; nu < DIR; nu++)
      delta += GAUGE_LOAD<atype, Real>(
          array,
          neighborIndexMinusOne(x, nu, DEVPARAMS::Grid) +
              nu * DEVPARAMS::Volume,
          offset);
  ...
  if (idd < DEVPARAMS::Volume) {
    ...
    for (int nu = 0; nu < 4; nu++) {
      ...
      msun g0 = (GX_LOAD_DAGGER< atypegx, Real>(
          gx, neighborIndexPlusOne(x, nu, DEVPARAMS::Grid), DEVPARAMS::Volume));
      U = U_temp * g0;
...
public:
  GaugeFix(gauge &pgauge, gauge GX, GaugeFixArg<Real> &arg)
    ...
    if (pgauge.EvenOdd())
      errorCULQCD("gauge cannot be in even/odd format...");
    if (GX.EvenOdd())
      errorCULQCD("gx not set in even/odd format...");
    kernel_pointer = NULL;
...
__global__ void kernel_gauge_fix_U_NEW(complex *array, complex *delta,
  ...
  msun de = DELTA_LOAD< atypedelta, Real>(delta, idd); //, arg.threads);
  msun g = msun::unit();
  g += de * half_alpha;
  reunit_link<Real>(&g);
  int x[4];
  ...
  for (int nu = 0; nu < 4; nu++) {
    ...
    de = DELTA_LOAD< atypedelta, Real>(
        delta, neighborIndexPlusOne(x, nu, DEVPARAMS::Grid));
    msun g0 = msun::unit();
    g0 += de * half_alpha;
    reunit_link<Real>(&g0);
    U = U_temp * g0.dagger();
...
complex GaugeFixingFFT(gauge _pgauge, int DIR, Real alpha, bool landautune,
  ...
  if (_pgauge.EvenOdd()) {
    COUT
        << "Only implemented for normal order arrays.\nNo gauge fixing applied."
        << endl;
    return complex::make_complex(9999., 999999999999.);
  }
