...
public:
  MPloop(gauge &arrayin, gauge &arrayout)
  ...
  void stat() {
    COUT << "MPloop:  " << time() << " s\t" << bandwidth() << " GB/s\t"
         << flops() << " GFlops" << endl;
  }
...
template <class Real> struct PotPLoopArg {
  complex *array;
  complex *pot;
  int radius;
};

template <class Real, bool ppdagger> class PotPLoop : Tunable {
private:
    ...
    if (id < DEVPARAMS::tstride) {
      for (int mu = 0; mu < NDIMS - 1; mu++) {
        msun pldown = GAUGE_LOAD<atypein, Real>(
            arg.array, Index_NDs_Neig_NM(id, mu, r), DEVPARAMS::tstride);
        ;
        if (ppdagger)
          value += plup.trace() * pldown.dagger().trace();
        else
          value += plup.trace() * pldown.trace();
      }
    }
    complex aggregate =
        BlockReduce(temp_storage).Reduce(value, Summ<complex>());
    if (threadIdx.x == 0)
      CudaAtomicAdd(arg.pot + r, aggregate);
  }
}

template <class Real, bool ppdagger>
PotPLoop<Real, ppdagger>::PotPLoop(gauge &array, complex *pot, int radius)
    : array(array), pot(pot) {
...
#if (NCOLORS == 3)
    if (array.Type() == SOA)
      LAUNCH_KERNEL(kernel_calc_polyakovloop, tp, stream, arg, SOA, Real,
                    ppdagger);
    if (array.Type() == SOA12)
...
void PotPLoop<Real, ppdagger>::Run(const cudaStream_t &stream) {
  ...
  CUDA_SAFE_CALL(cudaMemcpy(pot, arg.pot, (arg.radius + 1) * sizeof(complex),
...
template <class Real, bool ppdagger> void PotPLoop<Real, ppdagger>::Run() {
  return Run(0);
}
template <class Real, bool ppdagger> double PotPLoop<Real, ppdagger>::time() {
  return timesec;
}
template <class Real, bool ppdagger> void PotPLoop<Real, ppdagger>::stat() {
  COUT << "PotPLoop:  " << time() << " s\t" << bandwidth() << " GB/s\t"
       << flops() << " GFlops" << endl;
}
template <class Real, bool ppdagger>
void PotPLoop<Real, ppdagger>::printValue() {
  for (int r = 0; r <= arg.radius; r++)
    printfCULQCD("R = %d: < %.12e : %.12e : %.12e >\n", r, pot[r].real(),
                 pot[r].imag(), pot[r].abs());
}
template <class Real, bool ppdagger>
long long PotPLoop<Real, ppdagger>::flop() const {
  return 0;
}
template <class Real, bool ppdagger>
long long PotPLoop<Real, ppdagger>::bytes() const {
  return 0;
}
template <class Real, bool ppdagger> double PotPLoop<Real, ppdagger>::flops() {
  return ((double)flop() * 1.0e-9) / timesec;
}
template <class Real, bool ppdagger>
double PotPLoop<Real, ppdagger>::bandwidth() {
...
template void PotPL<float>(gauges array, complexs *pot, int radius,
                           bool ppdagger);
template void PotPL<double>(gauged array, complexd *pot, int radius,
                            bool ppdagger);
