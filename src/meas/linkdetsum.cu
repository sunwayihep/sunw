...
GaugeDetCUB<Real>::GaugeDetCUB(gauge &array) : array(array) {
  if (!array.EvenOdd())
    errorCULQCD("gauge array must be a even/odd array...");
  value = complex::zero();
...
template <class Real> GaugeDetCUB<Real>::~GaugeDetCUB() { dev_free(arg.value); }
template <class Real>
...
complex GaugeDetCUB<Real>::Run(const cudaStream_t &stream) {
  ...
  CUDA_SAFE_CALL(
      cudaMemcpy(&value, arg.value, sizeof(complex), cudaMemcpyDeviceToHost));
...
template <class Real> complex GaugeDetCUB<Real>::Run() { return Run(0); }
template <class Real> double GaugeDetCUB<Real>::time() { return timesec; }
template <class Real> void GaugeDetCUB<Real>::stat() {
  COUT << "Link Determinant Sum:  " << time() << " s\t" << bandwidth()
       << " GB/s\t" << flops() << " GFlops" << endl;
}
template <class Real> void GaugeDetCUB<Real>::printValue() {
  printfCULQCD("Link Determinant Sum: < %.12e : %.12e >\n", value.real(),
               value.imag());
}

template <class Real> long long GaugeDetCUB<Real>::flop() const {
  // NEED TO RECOUNT!!!!!!
#if (NCOLORS == 3)
  ...
  return (tmp_gs * NCOLORS * 8LL + tmp_det * (NCOLORS * 8LL + 11LL)) * 4LL *
         size * numnodes();
#endif
}
template <class Real> long long GaugeDetCUB<Real>::bytes() const {
  // NEED TO RECOUNT!!!!!!
#ifdef MULTI_GPU
...
template <class Real> double GaugeDetCUB<Real>::flops() {
  return ((double)flop() * 1.0e-9) / timesec;
}
template <class Real> double GaugeDetCUB<Real>::bandwidth() {
...
template <class Real> GaugeDet<Real>::GaugeDet(gauge &array) : array(array) {
  value = complex::zero();
  arg.value = (complex *)dev_malloc(sizeof(complex));
  arg.array = array.GetPtr();
  size = 1;
...
template <class Real> GaugeDet<Real>::~GaugeDet() { dev_free(arg.value); }
template <class Real> void GaugeDet<Real>::apply(const cudaStream_t &stream) {
  TuneParam tp = tuneLaunch(*this, getTuning(), getVerbosity());
...
template <class Real> complex GaugeDet<Real>::Run() { return Run(0); }
template <class Real> double GaugeDet<Real>::time() { return timesec; }
template <class Real> void GaugeDet<Real>::stat() {
  COUT << "Link Determinant Sum:  " << time() << " s\t" << bandwidth()
       << " GB/s\t" << flops() << " GFlops" << endl;
}
template <class Real> void GaugeDet<Real>::printValue() {
  printfCULQCD("Link Determinant Sum: < %.12e : %.12e >\n", value.real(),
               value.imag());
}

template <class Real> long long GaugeDet<Real>::flop() const {
  // NEED TO RECOUNT!!!!!!
#if (NCOLORS == 3)
  return (array.getNumFlop(true) + 82LL) * 4LL * size * numnodes();
#else
...
template <class Real> long long GaugeDet<Real>::bytes() const {
  // NEED TO RECOUNT!!!!!!
#ifdef MULTI_GPU
  return (2LL + array.getNumParams()) * 4LL * size * numnodes() * sizeof(Real);
#else
...
template <class Real> double GaugeDet<Real>::flops() {
  return ((double)flop() * 1.0e-9) / timesec;
}
template <class Real> double GaugeDet<Real>::bandwidth() {
  return (double)bytes() / (timesec * (double)(1 << 30));
}
