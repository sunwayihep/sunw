...
DEVICE msun MO4_q1(WLOPArg<Real> arg, int id, int dir1, int dir2, int lx,
                   int ly) {
  int dmu[2] = {dir1, dir2};
  int ids = id;
  msun link;
  ...
  for (int ix = 0; ix < lx; ix++) {
    link *= GAUGE_LOAD<atype, Real>(
        arg.gaugefield, ids + dmu[0] * DEVPARAMS::Volume, DEVPARAMS::size);
    ids = Index_ND_Neig_NM(ids, dmu[0], 1);
  }
  for (int iy = 0; iy < ly; iy++) {
    link *= GAUGE_LOAD<atype, Real>(
        arg.gaugefield, ids + dmu[1] * DEVPARAMS::Volume, DEVPARAMS::size);
...
///////////////////////////////
template <class Real, ArrayType atype>
DEVICE msun MO4_q2(WLOPArg<Real> arg, int id, int dir1, int dir2, int lx,
                   int ly) {
  int dmu[2] = {dir1, dir2};
  int ids = id;
  msun link;
...
//////////////////////////////////////
template <class Real, ArrayType atype>
DEVICE msun MO4_q3(WLOPArg<Real> arg, int id, int dir1, int dir2, int lx,
                   int ly) {
  int dmu[2] = {dir1, dir2};
  int ids = id;
  msun link;
...
/////////////////////////////////////
template <class Real, ArrayType atype>
DEVICE msun MO4_q4(WLOPArg<Real> arg, int id, int dir1, int dir2, int lx,
                   int ly) {
  int dmu[2] = {dir1, dir2};
  int ids = id;
  msun link;
...
// MO4
template <class Real, ArrayType atype>
DEVICE msun MO4(WLOPArg<Real> arg, int id, int lx, int ly, msun link,
                int muvolume) {
  msun mop = msun::zero();
  {
    int dir1 = arg.mu;
    ...
    qu[0] = MO4_q1< Real, atype>(arg, id, mu[0], mu[1], lx, ly);
    qu[1] = MO4_q2< Real, atype>(arg, id, mu[0], mu[1], lx, ly);
    qu[2] = MO4_q3< Real, atype>(arg, id, mu[0], mu[1], lx, ly);
    qu[3] = MO4_q4< Real, atype>(arg, id, mu[0], mu[1], lx, ly);
    // ids is the antiquark location
    aqu[0] = MO4_q1< Real, atype>(arg, ids, mu[0], mu[1], lx, ly);
    aqu[1] = MO4_q2< Real, atype>(arg, ids, mu[0], mu[1], lx, ly);
    aqu[2] = MO4_q3< Real, atype>(arg, ids, mu[0], mu[1], lx, ly);
    aqu[3] = MO4_q4< Real, atype>(arg, ids, mu[0], mu[1], lx, ly);
#pragma unroll
    for (int i = 0; i < 4; i++) {
