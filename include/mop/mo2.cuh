...
DEVICE msun MO2(WLOPArg<Real> arg,int id, int lx1, int ly, int lx2, int muvolume){
	...
	for(int il=0;il<2;il++){
		...
		//refreshing variables
			link0=msun::identity();
			ids[0]=Index_ND_Neig_NM(id, dmu[il], lx1, dmu1[il],ly);//1st up, 2nd forward
			for(int ix2=0;ix2<lx2;ix2++){
				ids[0]=Index_ND_Neig_NM(ids[0], dmu[il],-1);
				link0*=GAUGE_LOAD_DAGGER<UseTex, atype, Real>(arg.gaugefield, ids[0]+dmu[il]*DEVPARAMS::Volume, DEVPARAMS::size);
			}
		...
		ids[1]=Index_ND_Neig_NM(id, dir1, arg.radius, dmu[il], lx1);//right side coordinates
		{
		...
		// refreshing variables
			link0=msun::identity();
			ids[0]=Index_ND_Neig_NM(id, dmu[il],lx1, dmu1[il],-ly);//2nd part is backward
			for(int ix2=0;ix2<lx2;ix2++){
				ids[0]=Index_ND_Neig_NM(ids[0], dmu[il],-1);
				link0*=GAUGE_LOAD_DAGGER<UseTex, atype, Real>(arg.gaugefield, ids[0]+dmu[il]*DEVPARAMS::Volume, DEVPARAMS::size);
			}
...
/*refreshing coordinates*/
		...
		// 3p down
			link0=msun::identity();
			ids[0]=Index_ND_Neig_NM(id, dmu[il], -lx1, dmu1[il],ly);//1st down, 2nd forward
			for(int ix2=0;ix2<lx2;ix2++){
				ids[0]=Index_ND_Neig_NM(ids[0], dmu[il],-1);
				link0*=GAUGE_LOAD_DAGGER<UseTex, atype, Real>(arg.gaugefield, ids[0]+dmu[il]*DEVPARAMS::Volume, DEVPARAMS::size);
			}
		...
		// 3p down
			link0=msun::identity();
			ids[0]=Index_ND_Neig_NM(id, dmu[il],-lx1, dmu1[il],-ly);//1st down, 2nd backward 
			for(int ix2=0;ix2<lx2;ix2++){
				ids[0]=Index_ND_Neig_NM(ids[0], dmu[il],-1);
				link0*=GAUGE_LOAD_DAGGER<UseTex, atype, Real>(arg.gaugefield, ids[0]+dmu[il]*DEVPARAMS::Volume, DEVPARAMS::size);
			}
