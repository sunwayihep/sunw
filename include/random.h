...
public:
  ...
  void Backup() {
    ...
    if (Bytes() < mfree) {
      ...
      CUDA_SAFE_CALL(
          cudaMemcpy(backup_state, state, Bytes(), cudaMemcpyDeviceToDevice));
    } else {
      backup_Indev = false;
      backup_state = (cuRNGState *)safe_malloc(Bytes());
      CUDA_SAFE_CALL(
          cudaMemcpy(backup_state, state, Bytes(), cudaMemcpyDeviceToHost));
    }
  }
  void Restore() {
    ...
    if (backup_Indev) {
      CUDA_SAFE_CALL(
          cudaMemcpy(state, backup_state, Bytes(), cudaMemcpyDeviceToDevice));
      dev_free(backup_state);
    } else {
      CUDA_SAFE_CALL(
          cudaMemcpy(state, backup_state, Bytes(), cudaMemcpyHostToDevice));
...
*/
template <class Real> inline __device__ Real Random(cuRNGState &state) {
  Real res;
  return res;
}

template <> inline __device__ float Random<float>(cuRNGState &state) {
  return curand_uniform(&state);
}

template <> inline __device__ double Random<double>(cuRNGState &state) {
  return curand_uniform_double(&state);
}

template <class Real> inline __device__ Real RandomNormal(cuRNGState &state) {
  Real res;
  return res;
}

template <> inline __device__ float RandomNormal<float>(cuRNGState &state) {
  return curand_normal(&state);
}

template <> inline __device__ double RandomNormal<double>(cuRNGState &state) {
  return curand_normal_double(&state);
}

template <class Real> struct uniform {};
template <> struct uniform<float> {
  __device__ static inline float rand(cuRNGState &state) {
    return curand_uniform(&state);
  }
};
template <> struct uniform<double> {
  __device__ static inline double rand(cuRNGState &state) {
    return curand_uniform_double(&state);
  }
};
