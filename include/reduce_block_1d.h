...
template <typename T> inline __device__ T cuda_shfl_down(T val, int offset) {
...
template <typename T> inline __device__ T warpReduceSum(T val) {
  for (int offset = warpSize / 2; offset > 0; offset /= 2)
    val += cuda_shfl_down(val, offset);
  return val;
}

template <typename T> inline __device__ T blockReduceSum(T val) {
